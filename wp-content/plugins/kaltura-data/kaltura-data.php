...
function msp_Kalturadata_admin_page_screen()
{
    ...
    ?>
    <link href="<?php print get_template_directory_uri(); ?>/assets/css/bootstrap.css" rel="stylesheet"/>
    <style>
    .content{
        margin:20px;
    }
    ...
    <div class="content">
        <div class="button_group">
            <a href="<?php print home_url(); ?>/template.csv" class="btn btn-primary text-white download_template">Download csv template</a>
            <a href="<?php print $_SERVER['REQUEST_URI']; ?>&action=deleteall" onclick="return confirm('Are you sure?')" class="btn btn-danger text-white clear_all_movies">Delete all films</a>
        </div>
    ...
    if (!empty($_GET['action']) && ($_GET['action']=="deleteall")){
    ...
    if (!empty($_FILES)) {
        ...
        $client->setKS($ks);
        $id = 186281133;
        $partnerId = get_field('partner_id', 'option');
        ...
        if (($handle = fopen($_FILES["csvfile"]["tmp_name"], "r")) !== FALSE) {
            while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                ...
                if ($row>0) {
                  ...
                  // genre
                    if ($data[8]) {
                      $genre = explode(',', $data[8]);
                      if ($genre)
                        foreach ($genre as $cat) {
                            $cat_id = get_term_by('name', trim($cat), 'genre');
                            if ( $cat_id)
                                $table[$row]['genre'][] = $cat_id->term_id;
                            else {
                                $wpdocs_cat = array('cat_name' => trim($cat),   'taxonomy' => 'genre');
                                $wpdocs_cat_id = wp_insert_category($wpdocs_cat);
                                $table[$row]['genres'][] = $wpdocs_cat_id;
                            }
                        }
                    }
                  ...
                  // grade
                    if ($data[9]) {
                      $genre = explode(',', $data[9]);
                      if ($genre)
                        foreach ($genre as $cat) {
                            $cat_id = get_term_by('name', trim($cat), 'grade');
                            if ( $cat_id)
                                $table[$row]['grade'][] = $cat_id->term_id;
                            else {
                                $wpdocs_cat = array('cat_name' => trim($cat),   'taxonomy' => 'grade');
                                $wpdocs_cat_id = wp_insert_category($wpdocs_cat);
                                $table[$row]['grades'][] = $wpdocs_cat_id;
                            }
                        }
                    }
                  ...
                  // subject
                    if ($data[10]) {
                      $genre = explode(',', $data[10]);
                      if ($genre)
                        foreach ($genre as $cat) {
                            $cat_id = get_term_by('name', trim($cat), 'subject');
                            if ( $cat_id)
                                $table[$row]['subject'][] = $cat_id->term_id;
                            else {
                                $wpdocs_cat = array('cat_name' => trim($cat),   'taxonomy' => 'subject');
                                $wpdocs_cat_id = wp_insert_category($wpdocs_cat);
                                $table[$row]['subjects'][] = $wpdocs_cat_id;
                            }
                        }
                    }
                  ...
                  // topic
                    if ($data[11]) {
                      $genre = explode(',', $data[11]);
                      if ($genre)
                        foreach ($genre as $cat) {
                            $cat_id = get_term_by('name', trim($cat), 'topic');
                            if ( $cat_id)
                                $table[$row]['topic'][] = $cat_id->term_id;
                            else {
                                $wpdocs_cat = array('cat_name' => trim($cat),   'taxonomy' => 'topic');
                                $wpdocs_cat_id = wp_insert_category($wpdocs_cat);
                                $table[$row]['topics'][] = $wpdocs_cat_id;
                            }
                        }
                    }
                  ...
                  // tags
                    if ($data[12]) {
                      $genre = explode(',', $data[12]);
                      if ($genre)
                        foreach ($genre as $cat) {
                            $cat_id = get_term_by('name', trim($cat), 'post_tag');
                            if ( $cat_id)
                                $table[$row]['post_tag'][] = $cat_id->term_id;
                            else {
                                $wpdocs_cat = array('cat_name' => trim($cat),   'taxonomy' => 'post_tag');
                                $wpdocs_cat_id = wp_insert_category($wpdocs_cat);
                                $table[$row]['post_tags'][] = $wpdocs_cat_id;
                            }
                        }
                    }
                    ...
                    $table[$row]['dubbing_langs'] = $data[15];
                    $table[$row]['subtitles'] = $data[16];
                    $table[$row]['synopsis'] = $data[17];
                    $table[$row]['director'] = $data[18];
                    $table[$row]['cast'] = $data[19]; // actors
                    $table[$row]['thumb'] =  "https://cdnapisec.kaltura.com/p/" . $partnerId . "/thumbnail/entry_id/" . $table[$row]['kaltura_id'] . "/width/500/height/500/type/1/quality/100";
        ...
        foreach($table as $item){
            ...
            // print_r($item);
                ...
                update_field( "dubbing_lang", $item['dubbing_langs'], $post_id ); // +
                update_field( "subtitles", $item['subtitles'], $post_id ); // +
                update_field( "thumb", $item['thumb'], $post_id ); // +
                update_field( "synopsis", $item['synopsis'], $post_id ); // +
                update_field( "director", $item['director'], $post_id ); // +
                update_field( "actors", $item['cast'], $post_id ); // +

        }
        ?>
        <div class="alert alert-success"><?php print $count ?> films have been successfully added</div>
        <?php
