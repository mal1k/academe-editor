...
class TestMain implements IKalturaLogger{
	...
	*/
	...
	public function listActions(){
		...
		$filter->categoriesFullNameIn = "@zoomWebinarCategory@";
		$pager = new KalturaFilterPager();

		try{
			$result = $client->media->listAction($filter, $pager);
			//  echo '<pre>'.print_r($result,true)."</echo>";
		} catch(Exception $e){
			echo $e->getMessage();
		}	/*
...
$searchResults = $elasticsearchPlugin->eSearch->searchEntry($searchParams, null);
		 echo '<pre>'.print_r($searchResults,true)."</echo>";
		 /*
		$elasticSearchPlugin = KalturaElasticSearchClientPlugin::get($client);
		$searchParams = new KalturaESearchEntryParams();
