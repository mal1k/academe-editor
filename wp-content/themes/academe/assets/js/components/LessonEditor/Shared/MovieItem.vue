<template>
  <div class="movie-item">
    <img :src="movie_data.image" alt="img" />
    <div class="movie-item__body">
      ...
      <div class="movie-item__tags">
        <a
          ...
          v-for="(tag, index) in movie_data.tags.slice(0, 2)"
          :key="index"
          >#{{ tag.tag }}
        </a>
      </div>
      <div class="movie-item__footer">
        <button class="movie-item__btn" @click="addMovieToSlide">Add</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    addMovieToSlide() {
      //   const slide = this.store.slides.find(
      //     (s) => s.lesson_id === this.store.active_slide
      //   );

      //   if (!slide) {
      //     return alert("Select slide firstly");
      //   }

      this.$store.commit("LessonEditor/updateSlide", {
        lesson_id: this.store.active_slide,
        fields: {
          kaltura_id: this.movie_data.kaltura_id,
          play_from: 0,
          play_to: 0,
        },
      });
      // this.$store.state.LessonEditor.movie = this.movie_data;
      // slide.fields = {
      //   kaltura_id: this.movie_data.kaltura_id,
      //   play_from: 0,
      //   play_to: 0
