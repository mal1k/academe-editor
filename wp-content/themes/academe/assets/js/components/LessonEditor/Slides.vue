    ...
    <div class="slides-content">
        <div class="slide-list">
            <div class="slide-container">
                <div @click="new_slide_modal = true" class="add-slide-btn">
                    <svg class="icon " width="15" height="15" viewBox="0 0 15 15" fill="#51acfd" id="plusBlue" xmlns="http://www.w3.org/2000/svg"><path class="cls-1" d="M7.5,15a1,1,0,0,1-1-1V1a1,1,0,0,1,2,0V14A1,1,0,0,1,7.5,15Z"></path><path class="cls-1" d="M14,8.5H1a1,1,0,0,1,0-2H14a1,1,0,0,1,0,2Z"></path></svg>
                    <span>Add New Slide</span>
                </div>
            </div>
            <draggable v-model="store.slides" v-bind="dragOptions" @start="drag = true" @end="drag = false" handle=".draggable">
                <transition-group type="transition" :name="!drag ? 'flip-list' : null">
                    <div v-for="(slide, index) in store.slides"
                         :key="slide.lesson_id"
                         class="slide-container slide-preview-wrap"
                         :class="{'active': store.active_slide === parseInt(slide.lesson_id)}"
                         @click="store.active_slide = parseInt(slide.lesson_id)">
                        <div class="slide-number"><span>{{parseInt(index)+1}}</span></div>
                        <div class="slide-preview flex-builder" :class="{'active': store.active_slide === parseInt(slide.lesson_id)}">
        ...
        <div class="slide-content">
            <div class="aspect-ratio-box">
                <div class="aspect-ratio-box-inside">
                    <template v-if="store.active_slide !== null">
                        <template v-if="store.slides[activeSlideIndex].template === 'template1'">
                            <div class="slide-template-preview flex-builder">
                                <div class="row h-30">
                                    <div class="col w-100 text-title"
                                        :style="{
                                            backgroundColor: store.slides[activeSlideIndex].fields.template1_text1_fill_color,
                                        }"
                                        @click="changeActiveBlockMeta(store.slide_templates.template1.template1_text1)">

                                        <div v-if="store.slides[activeSlideIndex].fields.template1_text1_text == null" class="content-type">Text</div>
                                        <div v-else class="formatted-text" :style="{
                                            fontFamily: store.slides[activeSlideIndex].fields.template1_text1_font,
                                            fontSize: store.slides[activeSlideIndex].fields.template1_text1_font_size,
                                            lineHeight: '1.1em',
                                            color: store.slides[activeSlideIndex].fields.template1_text1_text_color,
                                            fontWeight: store.slides[activeSlideIndex].fields.template1_text1_font_weight,
                                            textAlign: store.slides[activeSlideIndex].fields.template1_text1_text_align,
                                        }" v-html="store.slides[activeSlideIndex].fields.template1_text1_text">

                                        </div>
                                        <slide-block-corners v-if="checkActiveBlockMeta(store.slide_templates.template1.template1_text1)"/>
                                ...
                                <div class="row h-70">
                                    <div class="col w-50 free-text"
                                        :style="{
                                            backgroundColor: store.slides[activeSlideIndex].fields.template1_text2_fill_color,
                                        }"
                                        @click="changeActiveBlockMeta(store.slide_templates.template1.template1_text2)">

                                        <div v-if="store.slides[activeSlideIndex].fields.template1_text2_text == null" class="content-type">Text</div>
                                        <div v-else class="formatted-text" :style="{
                                            fontFamily: store.slides[activeSlideIndex].fields.template1_text2_font,
                                            fontSize: store.slides[activeSlideIndex].fields.template1_text2_font_size,
                                            lineHeight: '1.1em',
                                            color: store.slides[activeSlideIndex].fields.template1_text2_text_color,
                                            fontWeight: store.slides[activeSlideIndex].fields.template1_text2_font_weight,
                                            textAlign: store.slides[activeSlideIndex].fields.template1_text2_text_align,
                                        }" v-html="store.slides[activeSlideIndex].fields.template1_text2_text">

                                        </div>
                                        <slide-block-corners v-if="checkActiveBlockMeta(store.slide_templates.template1.template1_text2)"/>

                                    </div>
                                    <div class="col w-50 media"
                                         @click="changeActiveBlockMeta(store.slide_templates.template1.template1_media1)">

                                        <div v-if="store.slides[activeSlideIndex].fields.template1_media1_image == null" class="content-type">Media</div>
                                        <template v-else>
                                            <img v-if="store.slides[activeSlideIndex].fields.template1_media1_image" :src="store.slides[activeSlideIndex].fields.template1_media1_image" style="width: 100%; height: 100%; object-fit: cover;"/>
                                        </template>


                                        <slide-block-corners v-if="checkActiveBlockMeta(store.slide_templates.template1.template1_media1)"/>

                                    </div>
                                </div>
                            </div>
                        </template>
                        <template v-if="store.slides[activeSlideIndex].template === 'template2'">
                            <div class="slide-template-preview flex-builder">
                                <div class="row h-30">
                        ...
                        <template v-if="store.slides[activeSlideIndex].template === 'template3'">
                            <div class="slide-template-preview flex-builder">

                            </div>
                        </template>
                    </template>
                    <div v-else @click="new_slide_modal = true" class="add-slide-btn">
                        <svg class="icon " width="15" height="15" viewBox="0 0 15 15" fill="#51acfd" id="plusBlue" xmlns="http://www.w3.org/2000/svg"><path class="cls-1" d="M7.5,15a1,1,0,0,1-1-1V1a1,1,0,0,1,2,0V14A1,1,0,0,1,7.5,15Z"></path><path class="cls-1" d="M14,8.5H1a1,1,0,0,1,0-2H14a1,1,0,0,1,0,2Z"></path></svg>
                        <span>Add New Slide</span>
                    </div>
        ...
        <div class="slide-meta">
            <div v-if="store.active_block_meta !== null" class="block-meta">
                ...
                <div class="body">
                    <div v-for="(field, name) in store.active_block_meta.fields" :key="name" class="input-row">
                        <slide-block-input :property_name="name" :field="field"/>
                    </div>
                </div>
            </div>
            <el-tabs v-if="store.active_slide !== null && store.active_block_meta == null" v-model="active_slide_meta_tab" @tab-click="">
                <el-tab-pane label="Layout" name="layout">
                    <div class="container-25px slide-templates flex-builder">
                        <template1 @click.native="store.slides[activeSlideIndex].template = 'template1'"
                                   :class="{'active' : store.slides[activeSlideIndex].template === 'template1'}" class="clickable">
                        </template1>
                        <template2 @click.native="store.slides[activeSlideIndex].template = 'template2'"
                                   :class="{'active' : store.slides[activeSlideIndex].template === 'template2'}" class="clickable">
                        </template2>
                        <template3 @click.native="store.slides[activeSlideIndex].template = 'template3'"
                                   :class="{'active' : store.slides[activeSlideIndex].template === 'template3'}" class="clickable">
                        </template3>
                    </div>
                </el-tab-pane>
        ...
        <el-dialog title="Select Slide" width="780px" :visible.sync="new_slide_modal">
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            activeSlideIndex() {
                var _this = this;
                return this.store.slides.map(function(x) {return x.lesson_id; }).indexOf(_this.store.active_slide);
            },
        },
        methods: {
            getSlideTemplates() {
                axios.get('/wp-json/academe/v1/slide-templates?_wpnonce=' + wpApiSettings.nonce ).then(res => {
                    this.store.slide_templates = res.data;
                });
            },
            getTemplateFields(template_name) {
                ...
                //for (const [template_key, template] of Object.entries(this.store.slide_templates)) {
                    var block_fields = {};
                    for ( const [block_key, block] of Object.entries(this.store.slide_templates[template_name]) ) {
            ...
            addSlideTextImage() {
                // Get ACF fields names for the selected template
                let fields = this.getTemplateFields('template1');

                let new_slide = {
                    lesson_id: this.store.slides.length, //should be replaced with post ID
                ...
                this.store.slides.push(new_slide);

                // Select last slide after creation
                this.store.active_slide = this.store.slides[this.store.slides.length - 1].lesson_id;

                // Close slide type selection modal
                this.new_slide_modal = false;
            },
            removeSlide(index) {
            ...
            changeActiveBlockMeta(meta_obj) {
                this.store.active_block_meta = (this.store.active_block_meta === meta_obj) ? null : meta_obj;
            },
