              ...
              <div class="slide-template-preview flex-builder">
                <div class="row h-30">
                  <div
                    ...
                    :style="{
                      backgroundColor:
                        activeSlide.fields.template1_text1_fill_color,
                    }"
                    @click="
    ...
    <div class="slide-meta">
      ...
      >
        <el-tab-pane label="Movie" name="movie">
          <div
            v-if="activeSlide.fields && activeSlide.fields.kaltura_id"
            class="container-25px flex-builder"
          >
            <MovieOptions v-bind:kalturaId="activeSlide.fields.kaltura_id" />
          </div>
...
export default {
  ...
  mounted() {
    ...
    this.getMyMovies();
    this.addSlideVideo(); // TODO: Remove
  },
  computed: {
    store() {
  ...
  methods: {
    ...
    getSlideTemplates() {
      axios
        .get("/academe/v1/slide-templates?_wpnonce=" + wpApiSettings.nonce)
        .then((res) => {
    ...
    addSlide(slide) {
      const lesson_id = Date.now();
      // Add slide with empty structure to the slides list
      this.store.slides.push({ ...slide, lesson_id });

      // Select last slide after creation
      this.store.active_slide = lesson_id;
