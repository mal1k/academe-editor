        ...
        <div class="movie-poster">
            <?php if(is_user_logged_in()) { ?>
                <?php if (is_user_in_role('student')) { ?>
                    <div class="start-watch start-movie-preview" data-movie-id="<?php echo $custom_fields['movie_id']->ID; ?>" data-mode="advanced">
            ...
            <img class="poster-image" src="<?php echo get_movie_thumbnail($movie_fields['kaltura_id']); ?>" />
        </div>
        <div class="movie-info">
            <div class="movie-top">
                <div class="title-wrap">
                    <h1 class="title" id="title_id" movie_id="<?php echo $custom_fields['movie_id']->ID; ?>"><?php the_title(); ?></h1>
                    <?php if (1) { ?>
                ...
                <?php if(is_user_logged_in() && !is_user_in_role('student')) { ?>
                    <div class="movie-actions">
                        ...
                        <div style="position: absolute; text-align: center; max-width: 120px; right: 40px; margin-top: -10px; right: 80px; background-color: #F98C40; border-radius: 3px;">
                            <a href="/lesson-editor?movie_id=<?php echo $custom_fields['movie_id']->ID; ?>" class="create-lesson-btn" style="background-color: #F98C40; border: 0; border-radius: 3px;">
                                    <p style="position: relative; margin-left: -28px;">Create a lesson</p> 
                                    <svg style="position: absolute; right: 0;" width="31" height="31" viewBox="0 0 31 31" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
            ...
            <div class="meta">
                <div class="year"><?php echo isset($movie_fields['year']) ? $movie_fields['year'] : ''; ?></div>
                <div class="duration">
                    <?php icon('duration', 'icon-light-gray');
                    if (isset($movie_fields['duration'])) {
                        $duration = explode(":", $movie_fields['duration']);
            ...
            <span class="genres-list">
                <?php $genres = wp_get_post_terms($custom_fields['movie_id']->ID, 'genre', ['fields' => 'names']);
                if ($genres) {
            ...
            <?php if(isset($movie_fields['director'])) { ?>
                <div class="info-subtitle"><?php _e('Directed by', 'academe-theme'); ?></div>
            <div class="directed"><?php echo $movie_fields['director']; ?></div>
            <?php } ?>
          ...
          <?php if ( !empty($teaching_guides) ) : ?>
            <div style="position: absolute; text-align: center; max-width: 120px; right: 80px; margin-top: -150px;">
                <a href="<?php echo $guideLink; ?>" class="create-lesson-btn" style="background-color: #D04A07; border: 0;">
                    <p style="position: relative; margin-left: -28px;">Teaching Guide</p> 
                    <svg style="position: absolute; right: 0px;" width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
